...
page = st.sidebar.selectbox('Choose your page', ['bookings', 'users', 'rooms'])
...
if page == 'users':
    ...
    # ユーザーの登録と会議室の登録のどちらかが済んでいない場合は表示しない
    if len(users)==0 or len(rooms)==0:
        st.error('ユーザーもしくは会議室の登録が済んでいません。')
    else:
        st.write('## 会議室一覧の表示')
        df_rooms = pd.DataFrame(rooms)
        df_rooms.columns = ['会議室名', '定員', '会議室ID']
        ...
        st.write('## 会議室の予約')
        # FastAPIにdatetime型のデータを送る時はstr型でないといけない
        # .isoformat()を用いるとISO形式のstr型として送ることができる
        with st.form(key='booking'):
            ...
            booked_num: int = st.number_input('予約人数', step=1, min_value=1, max_value=max([room['capacity'] for room in rooms_name.values()]))
